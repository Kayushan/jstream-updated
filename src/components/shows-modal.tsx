...
import { getMobileDetect, getYear, getSlug } from '@/lib/utils';
...
const ShowModal = () => {
  ...
  console.log('ShowModal render - open:', modalStore.open, 'show:', modalStore.show?.title || modalStore.show?.name); // Debug log
  ...
  React.useEffect(() => {
    setOptions((state: Record<string, object>) => ({
      ...state,
      playerVars: { ...state.playerVars, mute: 0 },
    }));
    void handleGetData();
  }, []);
  ...
  const handleHref = (): string => {
    ...
    const name = modalStore.show.title || modalStore.show.name || '';
    ...
    if (isAnime) {
      ...
      return `/watch/${type}/${prefix}-${slug}`;
    }
    
    return `/watch/${type}/${slug}`;
  };
        ...
        <div className="video-wrapper relative aspect-video">
          ...
          {trailer && (
            <Youtube
              ...
              iframeClassName="absolute inset-0 h-full w-full opacity-0 transition-opacity duration-300"
            />
          )}
