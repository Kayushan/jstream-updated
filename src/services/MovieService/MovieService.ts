...
class MovieService extends BaseService {
  ...
  static async findMovieByIdAndType(id: number, type: string) {
  ...
  static async searchMovies(query: string, page?: number) {
    const { data } = await this.axios(baseUrl).get<TmdbPagingResponse>(
      `/search/multi?query=${encodeURIComponent(query)}&page=${
        page ?? 1
      }&language=en-US`,
    );
